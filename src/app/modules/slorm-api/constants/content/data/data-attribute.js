...
function setStat(trait, index, stat) {
    const value = trait.values[index];
    if (value) {
        value.stat = stat;
    }
}
function setSynergyPrecision(trait, index, precision) {
    const value = trait.values[index];
...
function allowSynergyToCascade(trait, index) {
    const value = trait.values[index];
    if (value && (0, utils_1.isEffectValueSynergy)(value)) {
...
function synergySetAllowMinMax(trait, index, allowMinMaw) {
    const value = trait.values[index];
    if (value && (0, utils_1.isEffectValueSynergy)(value)) {
